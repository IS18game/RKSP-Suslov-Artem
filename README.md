...
7. **Проверьте ClickHouse:**
   ...
   - Выполните запрос (ОБЯЗАТЕЛЬНО используйте обратные кавычки ` вокруг имен с кириллицей):
   ```sql
   SELECT * FROM `агрегаты_событий_заказов` ORDER BY `дата_и_время_записи` DESC;
   ```
   
   **ВАЖНО:** 
   - Используйте обратные кавычки (`) - это символ на клавиатуре слева от цифры 1 (обычно вместе с тильдой ~)
   - НЕ используйте обычные одинарные кавычки (') или двойные кавычки (")
   - Без обратных кавычек ClickHouse выдаст ошибку: `Code: 62. DB::Exception: Unrecognized token`
   - Если результат пустой - убедитесь, что вы вызвали POST `/api/v1/events/count` в шаге 6
